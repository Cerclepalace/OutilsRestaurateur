-- =============================================================================
-- BOBO PARIS — 0013 Cart line thumbnails
--
-- Images are anchored to the first variant of each colourway, so the previous
-- lookup — "this exact variant, or no variant at all" — matched nothing for any
-- other size. A cart line for Gauthier in M had no thumbnail at all.
--
-- The lookup now falls back by colourway, then to any product image, so a
-- product with pictures always shows one.
-- =============================================================================
    ...
    select
      jsonb_build_object(
        ...
        'image_url', (
          ...
          order by
            case
              when iv.id = v.id then 0                                    -- this exact variant
              when iv.color_name is not distinct from v.color_name then 1 -- same colourway
              when img.variant_id is null then 2                          -- product-level image
              else 3                                                      -- any other colourway
            end,
            img.position
          limit 1
        ),
